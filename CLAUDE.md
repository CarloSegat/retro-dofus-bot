...
- `calibrate_cells.py [N]` — proxy-driven cell→screen calibration. Click N cells,
  ...
  `config.json.cell_calibration`.
- `hunter.py` — proxy-driven hunter. First mob from `mob_groups` → cell→screen
  → click → wait `fight_start` → run combat cycle (currently still imports
  the visual `run_combat_cycle` from `fight.py`; see TODOs).
- `idle_wait_and_sacrid.py` — engage nearest mob, then per turn: walk
  adjacent to the closest alive enemy from `fight_entities`, cast Sacrid
  Foot (`sacrid_foot_hotkey`) once if adjacent and `ap >= sacrid_foot_ap_cost`,
  pass. One cast per turn (game cooldown). All targeting/AP off proxy
  GTM data, NOT screen detection.
- `fight_bronze.py` — legacy bronze-protector scripted fight (visual
  detection). Kept for reference; not used by the Sacrid loop.
...
```bash
# Terminal 1: proxy
sudo go run ./cmd/proxy --events 127.0.0.1:9999 2>&1 | tee /tmp/proxy.log
...
# Terminal 3: pick a hunter mode
python3 -u hunter.py
python3 -u idle_wait_and_sacrid.py
```
...
- `sacrid_foot_hotkey`: pyautogui key for Marx-Rockfeller's Sacrid Foot
  slot. **Required** — `idle_wait_and_sacrid.py` refuses to start if empty
  so a placeholder doesn't silently miscast.
...
- `sacrid_walk_wait_sec`: max wait for `my_cell` to settle after a walk
  click (default 2.0).
- Miner-era / bronze-fight knobs (`bronze_*`, `offset`, `orange_rgb`, …)
  are unused by the Sacrid loop but kept for `fight_bronze.py`.

## Things that have bitten us

- **`pyautogui.moveTo` before a pynput right-click silently breaks Dofus
  retro on X11.** See `fight_bronze._right_click` for the hybrid pattern.
  Memory `feedback_right_click_pynput.md`.
